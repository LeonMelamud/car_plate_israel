...
*   **Search Vehicles (`search_vehicles`)**: Search for vehicles. Currently supports filtering by model (`degem_nm`) and license plate (`mispar_rechev`).
*   **List Available Licenses (`list_available_licenses`)**: Retrieve a list of all available licenses for datasets on data.gov.il.
*   **Get Vehicle Dataset License (`get_vehicle_dataset_license`)**: Fetch the specific license information for the vehicle dataset being used by this MCP.
...
3.  **Available Tools:**
    Once configured, the AI assistant should be able to list and use the following tools (the prefix `israel-vehicle-data` is derived from the MCP name):
    *   `israel-vehicle-data.get_vehicle_by_plate`
        *   Args: `mispar_rechev: str` (license plate number)
        *   Returns: Vehicle details or an error.
    *   `israel-vehicle-data.search_vehicles`
        *   Args: `degem_nm: Optional[str]`, `mispar_rechev: Optional[str]`, `limit: int = 10`, `offset: int = 0`
        *   Returns: A list of matching vehicles or an error.
    *   `israel-vehicle-data.list_available_licenses`
        *   Args: None
        *   Returns: A list of all dataset licenses.
    *   `israel-vehicle-data.get_vehicle_dataset_license`
        *   Args: None
        *   Returns: License details for the specific vehicle dataset.
